...
*   **Learn & Earn Virtual Sats:** Gain knowledge about Bitcoin concepts from "Knowledge NPCs" and earn in-game virtual Satoshis. These sats can then be spent on interactions like querying the AI Demon.
...
*   **AI Demon Chat:** Interact with the Demon character ([`www/img/characters/demon.png`](www/img/characters/demon.png)) for chat-based conversations powered by Gemini AI via the `/api/ask-demon` endpoint in [`api/index.js`](api/index.js:1). Discuss topics like Bitcoin, Lightning Network, and Lightning Service Providers (LSPs). This interaction costs 1 virtual sat per query.
...
*   **Backend:** An Express server ([`api/index.js`](api/index.js:1)) handles Socket.IO connections, manages player states (positions), and broadcasts updates.
*   **Frontend:** The Phaser client ([`www/js/index.js`](www/js/index.js:1)) connects to the Socket.IO server, sends local player movement, and receives updates about other players to render them on screen.
...
*   **Interactive NPCs:**
    ...
    *   **AI Demon Chat:** Interact with the Demon NPC ([`www/img/characters/demon.png`](www/img/characters/demon.png)) for AI-powered chat on Bitcoin, Lightning Network, and LSPs. Costs 1 virtual sat per query.
...
*   **Scoring & Virtual Sats:** Earn virtual Sats for completing lessons and correct quiz answers. Spend these sats to interact with certain NPCs like the AI Demon.
...
Refer to **[`AmazonQContext.md`](AmazonQContext.md:1)** for the full AI collaboration strategy.
...
2.  **Start the Frontend Development Server (with Proxy):**
    ...
    ```
    (The `--cors` flag allows cross-origin requests from the game client to the API, and `--proxy http://localhost:3001` forwards requests for paths like `/api/*` and Socket.IO connections from `http://localhost:8080` to the backend server running on `http://localhost:3001`.)
